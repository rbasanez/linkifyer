...
class Item:
    ...
    def delete_item(self, user_id, item_hash):
        """
        Deletes an item from the database.

        Parameters:
        - user_id: User ID associated with the item
        - item_hash: Hash of the item to delete
        """
        try:
            success, result = self._db.execute("DELETE FROM Items WHERE user_id=? AND item_hash=?", (user_id, item_hash))
            if success: 
                self._logger.info(f"{self._method_path()}: {success}")
            else: 
                self._logger.error(f"{self._method_path()}: {result}")
